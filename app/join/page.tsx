...
export default function JoinPage() {
  ...
  const handleVerifyCode = () => {
    if (!family) { setCodeError("No family board found on this device."); return; }
    if (joinCode.trim().toLowerCase() !== (family.joinCode || "").toLowerCase()) {
      setCodeError("Incorrect code. Ask a family admin for the join code shown on the board.");
      return;
    }
    setCodeError("");
    setStep(1);
  ...
  return (
    <div className="min-h-screen bg-amber-50 flex items-center justify-center p-6">
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-sm p-6">
        {step === 0 && (
          <>
            <h1 className="text-2xl font-bold text-stone-800 mb-1">Join BoardRoom</h1>
            <p className="text-sm text-stone-400 mb-6">
              Enter the family join code shown on the board to create your account here on the iPad.
            </p>
            <div className="bg-amber-50 border border-amber-200 rounded-xl p-3 mb-5 text-xs text-stone-600">
              📱 <strong>Have a phone?</strong> Download <strong>BoardRoom Remote</strong> instead for full access from anywhere.
            </div>
            <label className="text-xs font-semibold text-stone-400 uppercase tracking-wide mb-2 block">Join Code</label>
            <input
              value={joinCode}
              onChange={e => setJoinCode(e.target.value)}
              placeholder="e.g. a8x3k-9mz2"
              className="w-full bg-stone-50 border border-stone-200 rounded-xl px-4 py-3 text-center text-lg font-mono tracking-widest text-stone-800 focus:outline-none focus:border-amber-400 mb-3"
              autoCapitalize="none"
            />
            ...
            <button
              ...
              className="w-full py-3 bg-amber-500 text-stone-900 rounded-xl font-bold hover:bg-amber-400 transition-colors disabled:opacity-40"
            >
              Continue →
            </button>
            <button onClick={() => router.replace("/")} className="w-full mt-3 text-sm text-stone-400 hover:text-stone-600 py-2">
              ← Back
            </button>
          </>
        )}
