    ...
    >
      ...
      >
          ...
          {directedMembers.length > 0 ? (
            directedMembers.map((m, i) => (
              <div
                ...
                style={{ backgroundColor: m.color, flex: 1 }}
              />
            ))
          ) : (
            <div className="rounded-t-lg w-full h-full" style={{ backgroundColor: author?.color || "#888" }} />
          )}
        </div>
        ...
        <p
          ...
          style={{ fontFamily: noteFont, fontSize: theme.fontStyle === "handwriting" ? "1.3rem" : undefined }}
        >
          {note.content}
        </p>
        ...
        <div className="flex items-center gap-1.5 mt-3 pt-2 border-t border-stone-200/50 dark:border-stone-700/50 flex-wrap">
          {directedMembers.length > 0 ? (
            ...
            <>
              <div className="w-3 h-3 rounded-full" style={{ backgroundColor: author?.color || "#888" }} />
              <span className="text-xs text-stone-400">{author?.name || "Unknown"}</span>
            </>
          )}
        </div>
      </div>
